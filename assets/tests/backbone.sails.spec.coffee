...
describe "Model", ->
  ...
  addTo = =>
    ...
    it "should add an id string", (done) ->
      ian = new Model name: "Ian"
      fred = new Model name: "Fred"
      all = Promise.all [ian.save(), fred.save()]
      all.then ->
        ian.populate("tests").addTo("tests", fred.id)
      .then ->
        tests = ian.get("tests", true)
        fredAgain = tests.findWhere name: "Fred"
        expect(fredAgain.id).toEqual(fred.id)
        done()
  ...
  removeFrom = =>
    ...
    it "should remove an id string", (done) ->
      ian = new Model name: "Ian"
      fred = new Model name: "Fred"
      all = Promise.all [ian.save(), fred.save()]
      all.then ->
        ian.populate("tests").addTo("tests", fred.id)
      .then ->
        ian.removeFrom("tests", fred.id)
      .then ->
        tests = ian.get("tests", true)
        fredAgain = tests.findWhere name: "Fred"
        expect(fredAgain).not.toBeTruthy()
        done()
